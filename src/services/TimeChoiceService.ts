...
import type { ContentLine, DisplayedLineV2, LineStatus } from "@/types";
...
export class TimeChoiceService {
  ...
  handleTimeChoice(time: string, lineId: string): void {
    ...
    if (!line || line.type !== "timeChoice") {
      console.log(
        "[TimeChoiceService] Early return: line not found or wrong type",
      );
      return;
    }
    ...
    console.log(
      "[TimeChoiceService] Converted to timeDisplay, insertIndex:",
      insertIndex,
    );

    // 插入后续内容
    const newLines = this._createDisplayedLines(
      matchedChoice.lines,
      insertIndex,
    );
    ...
    console.log("[TimeChoiceService] Updating display state with:", {
      ...
      firstFewLines: displayedLines
        .slice(0, Math.min(5, displayedLines.length))
        .map((l) => ({
          id: l.id,
          type: l.type,
    ...
    console.log("[TimeChoiceService] Display state updated, final state:", {
      displayedLinesCount:
        stateStore._internalDisplayState.displayedLines.length,
