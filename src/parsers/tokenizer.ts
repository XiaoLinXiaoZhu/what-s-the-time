import type { TokenizerOptions, TokenSegment } from "@/types/index";
...
export class ScriptTokenizer {
  ...
  private tokenizeSegment(
    ...
    // 分离主内容和子片段
    const { mainContent, subSegments } = this.extractSubSegments(
      remainingContent,
      metadata.id,
    );
  ...
  private extractSubSegments(
    ...
    // 规范化文本（延时简写转换等）
    const normalizedMainContent = mainContent.map((line) =>
      normalizeText(line),
    );
    const normalizedSubSegments = new Map<string, string[]>();

    subSegments.forEach((lines, id) => {
      normalizedSubSegments.set(
        id,
        lines.map((line) => normalizeText(line)),
      );
    });
