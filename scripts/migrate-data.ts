...
import type { ScriptSegmentV2, ContentLine, ChoiceLineV2, TimeChoiceLineV2 } from '@/types/script-v2'
...
function convertTimeChoiceLineV1ToV2(timeChoiceLine: any): TimeChoiceLineV2 {
  const choices = timeChoiceLine.choices.map((choice: any, index: number) => {
    if (choice.lines && choice.lines.length > 0) {
 ...
 * 提取所有子片段（从转换后的片段中）
 */
export function extractSubSegments(segments: ScriptSegmentV2[]): Array<{ id: string; lines: any[] }> {
 ...
 * 完整的迁移流程
 */
export function migrateSegmentV1ToV2(segment: ScriptSegment): ScriptSegmentV2[] {
  // 1. 转换主片段
